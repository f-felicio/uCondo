...
export default function Home({navigation}) {
  ...
  const [allowsAdd, setAllowsAdd] = useState('');
  ...
  const sendRegister = async () => {
    ...
    const verify = await register(editedPlan, storage);
    if (verify.exist) {
      setErrorMsg('Código já cadastrado.');
    }
    if (verify.empty) {
      ...
      <MainContainer>
        <Header>
          <Label onPress={() => navigation.goBack()}>
            <Icon name="left" size={22} color="#fff" /> Nova Conta
          </Label>
          <BtnAdd onPress={sendRegister}>
            <Label>
